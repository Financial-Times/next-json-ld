...
function orderByPredicateName(a, b) {
	...
	if (weightA === weightB) {
		const diffSlashes =
			countSlashes(a.relativeUrl) - countSlashes(b.relativeUrl);
		if (diffSlashes === 0) {
...
function getHierarchyAnnotations(annotations) {
	...
	const highlLevelAnnotations = annotations
		.map((annotation) => {
	...
	return highlLevelAnnotations
		.sort(orderByPredicateName)
		.slice(0, maxTagNumber);
}
...
function getItems(content) {
	...
	if (lastItem) {
		items = items
			.filter((item) => item.prefLabel !== lastItem.prefLabel)
			.slice(0, maxTagNumber - 1);
		items.push(lastItem);
		return items;
	}
	return items;
